...
public class ModBiomeModifiers {
    ...
    public static void bootstrap (BootstrapContext<BiomeModifier> context) {
        ...
        context.register(DRAKIUM_ORE, new BiomeModifiers.AddFeaturesBiomeModifier(
                ...
                HolderSet.direct(placedFeatures.getOrThrow(ModPlacedFeatures.DRAKIUM_ORE_KEY)),
                GenerationStep.Decoration.UNDERGROUND_ORES
        ));

        context.register(DRAKIUM_ORE_END, new BiomeModifiers.AddFeaturesBiomeModifier(
                biomes.getOrThrow(BiomeTags.IS_END),
                HolderSet.direct(placedFeatures.getOrThrow(ModPlacedFeatures.DRAKIUM_END_ORE_KEY)),
                GenerationStep.Decoration.UNDERGROUND_ORES
        ));

        context.register(DRAKIUM_ORE_NETHER, new BiomeModifiers.AddFeaturesBiomeModifier(
                biomes.getOrThrow(BiomeTags.IS_NETHER),
                HolderSet.direct(placedFeatures.getOrThrow(ModPlacedFeatures.DRAKIUM_NETHER_ORE_KEY)),
                GenerationStep.Decoration.UNDERGROUND_ORES
        ));
    }

    private static ResourceKey<BiomeModifier> registerKey (String name) {
        return ResourceKey.create(NeoForgeRegistries.Keys.BIOME_MODIFIERS, DimensionalStorage.getResource(name));
    }
